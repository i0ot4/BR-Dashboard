...
namespace BR.Repositories
{
    public class ConstructionShopRepository : IConstructionShopRepository
    {
        ...
        public async Task<IResult<UserViewVM>> UpdateConstructionShopVM(CreateConstructionShopVM contractorVM, string id, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                else
                {
                    ...
                    if(item.UserType != UserTypes.ConstructionShop) return Result<UserViewVM>.Fail($"--- user not match: {id}---");
